...
import {
  ...
  Crown, Store, Save, Camera, Trash2,
} from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = {
        ...
        shopGstin:   shopGstin.trim().toUpperCase(),
        shopLogo,
      };
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setProfile({ ...(profile as any), shopLogo: url });
  ...
  const handleRemoveLogo = async () => {
    ...
    try {
      ...
      setShopLogo('');
      setProfile({ ...(profile as any), shopLogo: '' });
    ...
    <div className="max-w-lg mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-5 flex items-center gap-4 mb-4">
        <div className="relative w-14 h-14 rounded-2xl overflow-hidden bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center shrink-0">
          {user?.photoURL
            ...
            : <span className="text-blue-600 dark:text-blue-400 font-bold text-xl">{user?.displayName?.[0]?.toUpperCase() || 'S'}</span>}
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-gray-900 dark:text-white truncate">{user?.displayName || 'Shop Owner'}</p>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-5 mb-4">
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              disabled={uploading}
              className="relative w-20 h-20 rounded-2xl bg-gray-50 dark:bg-slate-700 border-2 border-dashed border-gray-200 dark:border-slate-600 flex items-center justify-center hover:border-blue-300 dark:hover:border-blue-600 transition-all overflow-hidden disabled:opacity-60"
            >
              {shopLogo
            ...
            <div className="space-y-2">
              <button
                ...
                onClick={() => logoRef.current?.click()}
                disabled={uploading}
                className="text-xs font-medium text-blue-500 hover:text-blue-600 block disabled:opacity-50"
              >
                {uploading ? 'Uploading…' : shopLogo ? 'Change Logo' : 'Upload Logo'}
              </button>
              {shopLogo && !uploading && (
                <button type="button" onClick={handleRemoveLogo} className="text-xs text-red-400 hover:text-red-600 flex items-center gap-1">
                  <Trash2 size={10} /> Remove
                </button>
              )}
              <p className="text-xs text-gray-400 dark:text-slate-500">PNG, JPG · auto WebP</p>
            </div>
          </div>
        ...
        <div className="space-y-3">
          ...
          <Input label="Contact Number" value={shopPhone}   onChange={(e) => setShopPhone(e.target.value)}   placeholder="+91 98765 43210" type="tel" />
          <Input
          ...
          <p className="text-xs text-gray-400 dark:text-slate-500">
            Shop name, address, phone &amp; GSTIN appear on all PDF invoices.
          </p>
        </div>
      </div>
      ...
      <button
        ...
        className={`w-full flex items-center gap-4 p-4 rounded-2xl border mb-4 transition-all ${isActive ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800' : 'bg-gray-50 dark:bg-slate-800/50 border-gray-100 dark:border-slate-700'}`}
      >
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">
            {isActive && expiryDate ? `Renews ${expiryDate}` : `Subscribe for ${formatINR(399)}/month`}
          </p>
        </div>
      ...
      <p className="text-center text-xs text-gray-300 dark:text-slate-700 mt-8">Stockhold v2.0.0</p>
    </div>
  );
}
