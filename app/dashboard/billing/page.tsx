...
import {
  ...
  FileText, X, ArrowRight, ArrowLeft, Package,
  ScanLine, MessageCircle,
} from 'lucide-react';
...
import { Bill, BillItem, PaymentType } from '@/types';
...
// ─── Page wrapper (subscription guard) ──────────────────────────────────────
...
function BillingContent() {
  ...
  // store last bill for WhatsApp link
  ...
  const paymentStatus = paid === 0 ? 'pending' : paid >= subtotal ? 'successful' : 'half_paid';
  ...
  const handleBarcodeScanned = async (code: string) => {
    ...
    try {
      ...
      if (!p) { toast.error('Product not found for this barcode'); return; }
      if (p.quantity < 1) { toast.error(`${p.name} is out of stock`); return; }
  ...
  const handleAddProduct = (p: { id: string; name: string; sellingPrice: number; quantity: number }) => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const billData: Omit<Bill, 'id' | 'createdAt' | 'userId'> = {
        invoiceId,
        shopName:        profile?.shopName    || 'Stockhold',
        ...
        paymentType,
        paymentStatus:   paymentStatus as Bill['paymentStatus'],
        notes,
      };

      // 1. Save bill to Firestore
      ...
      for (const item of items) {
        ...
        if (product) {
          ...
          await updateProduct(user.uid, item.productId, { quantity: newQty });
        }
      }
      ...
      // 4. Store for WhatsApp link and reset
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-slate-400 text-sm mt-1">Search and add products to build a bill</p>
          </div>
          {items.length > 0 && (
            <span className="w-7 h-7 bg-blue-500 text-white text-xs font-bold rounded-full flex items-center justify-center">
              {items.length}
            </span>
          )}
        </div>
        ...
        {search.trim() ? (
          filteredProducts.length === 0 ? (
            <div className="text-center py-8 text-gray-400 dark:text-slate-600 text-sm mb-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-6">
              {filteredProducts.map((p) => (
                <button
                  ...
                  className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl p-3 text-left hover:border-blue-300 dark:hover:border-blue-700 hover:shadow-sm transition-all"
                >
                  {/* Squircle image */}
                  <div className="w-full aspect-square bg-gray-50 dark:bg-slate-700 rounded-xl overflow-hidden mb-2 flex items-center justify-center">
                    {p.imageUrl ? (
                  ...
                  <p className={`text-xs mt-0.5 ${p.quantity < 5 ? 'text-red-400' : 'text-gray-400 dark:text-slate-500'}`}>
                    {p.quantity} left
                  </p>
                </button>
              ))}
          ...
          <div className="text-center py-10 bg-gray-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 mb-6">
            ...
            <p className="text-gray-400 dark:text-slate-500 text-sm">
              Type a product name above or scan a barcode
            </p>
          </div>
        )}
                ...
                <div key={item.productId} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="w-6 h-6 rounded-lg border border-gray-200 dark:border-slate-600 flex items-center justify-center text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-700 transition-all"
                    >
                      <Minus size={10} />
                    </button>
                    <span className="text-sm font-bold text-gray-800 dark:text-gray-200 w-6 text-center">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => updateQty(item.productId, item.quantity + 1)}
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <h2 className="text-xl font-bold font-display text-gray-900 dark:text-white mb-5">
        Customer & Payment
      </h2>
        ...
        <div className="space-y-1.5 text-sm mb-3">
          {items.map((i) => (
            <div key={i.productId} className="flex justify-between text-gray-700 dark:text-gray-300">
              ...
              <span className="font-medium shrink-0">{formatINR(i.total)}</span>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4">
        ...
        <Input
          ...
          placeholder={`Total: ${formatINR(subtotal)}`}
          min="0"
        />
          ...
          <div className="bg-gray-50 dark:bg-slate-800 rounded-xl p-3 space-y-1.5 text-sm border border-gray-100 dark:border-slate-700">
            ...
            <div className="flex justify-between text-gray-500 dark:text-slate-400">
              ...
              <span className={`font-semibold ${remaining > 0 ? 'text-red-500' : 'text-green-500'}`}>
                {remaining > 0 ? formatINR(remaining) : 'Fully Paid ✓'}
              </span>
            </div>
            <div className="flex justify-between text-gray-500 dark:text-slate-400">
              <span>Status</span>
