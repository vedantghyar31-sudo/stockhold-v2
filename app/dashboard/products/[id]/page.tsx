...
export default function ProductDetailPage() {
  ...
  const handleDelete = async () => {
    if (!user || !product || !confirm('Delete this product? This cannot be undone.')) return;
    setDeleting(true);
    try {
  ...
  if (!product) return (
    <div className="text-center py-20">
      ...
      <p className="text-gray-500 dark:text-slate-400 font-medium">Product not found</p>
      <button
        onClick={() => router.back()}
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      {/* Image */}
      <div className="relative w-full bg-gray-100 dark:bg-slate-800 rounded-2xl overflow-hidden mb-5 border border-gray-100 dark:border-slate-700" style={{ maxHeight: '288px', aspectRatio: '1' }}>
        {product.imageUrl ? (
        ...
        {product.quantity < 5 && (
          <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-red-500 text-white text-xs font-semibold px-2.5 py-1 rounded-full shadow-sm">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-6 mb-4">
        ...
        {product.barcode && (
          <p className="text-xs text-gray-400 dark:text-slate-500 font-mono mb-4">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3">
            <p className="text-xs text-gray-500 dark:text-slate-400 mb-0.5">Cost Price</p>
          ...
          <div className={`rounded-xl p-3 ${product.quantity < 5 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-slate-700/50'}`}>
            <p className={`text-xs mb-0.5 ${product.quantity < 5 ? 'text-red-500' : 'text-gray-500 dark:text-slate-400'}`}>In Stock</p>
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400">
          Margin: <span className="font-semibold text-gray-700 dark:text-gray-200">{margin}%</span>
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex gap-3">
          <Button onClick={() => setShowSell(true)} className="flex-1 py-3">
            <ShoppingCart size={16} /> Quick Sell
          </Button>
          <Button onClick={handleAddToBill} variant="secondary" className="flex-1 py-3">
      ...
      {showSell && (
        <SellModal
          ...
          onSold={(updated) => { setProduct(updated); setShowSell(false); }}
        />
      )}
    </div>
  );
