...
export const generatePDF = async (
  ...
  const gstin = profile?.shopGstin   || bill.shopGstin   || '';
  ...
  // Logo (top-left)
  if (profile?.shopLogo) {
    try { doc.addImage(profile.shopLogo, 'WEBP', L, 12, 22, 22); } catch { /* skip */ }
  }
  if (!profile?.shopLogo) {
    doc.setFontSize(8);
  ...
  if (phone) doc.text(`Tel: ${phone}`, col2, y);
  if (phone) y += 6;

  // GSTIN on receipt (From section)
  if (gstin) {
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(40, 40, 40);
    doc.text(`GSTIN: ${gstin}`, col2, y);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(60, 60, 60);
    y += 6;
  }
